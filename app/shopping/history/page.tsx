        ...
        <AppLayout>
            <div className="space-y-6 animate-fade-in pb-12">
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => router.back()}
                        className="p-2 rounded-xl bg-card shadow-sm border border-border hover:bg-muted transition-colors"
                    >
                        <ArrowLeftIcon className="w-5 h-5 text-muted-foreground" />
                    </button>
                    <div>
                        <h1 className="text-2xl font-bold text-foreground">Transaction History</h1>
                        <p className="text-sm text-muted-foreground">Your deposits and shopping expenses</p>
                    </div>
                </div>
                    ...
                    <div className="p-4 border-b border-border">
                        <div className="flex gap-2 p-1 bg-muted rounded-xl">
                            <button
                                ...
                                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${activeTab === 'deposits' ? 'bg-card shadow-md text-primary scale-[1.02]' : 'text-muted-foreground hover:text-foreground'}`}
                            >
                                💰 Deposit History ({deposits.length})
                            </button>
                            <button
                                onClick={() => setActiveTab('expenses')}
                                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${activeTab === 'expenses' ? 'bg-card shadow-md text-primary scale-[1.02]' : 'text-muted-foreground hover:text-foreground'}`}
                            >
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-3">
                                {activeTab === 'deposits' && (
                                    deposits.length > 0 ? (
                                        deposits.map(d => (
                                            <div key={d._id} className="group p-4 bg-muted rounded-xl border border-border hover:border-primary-200 dark:hover:border-primary-500/30 transition-all">
                                                <div className="flex justify-between items-start mb-2">
                                                    <div>
                                                        <p className="font-bold text-card-foreground text-lg">৳{d.amount.toLocaleString()}</p>
                                                        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{d.paymentMethod}</p>
                                                    </div>
                                                ...
                                                <div className="flex justify-between items-end text-sm text-muted-foreground mt-2">
                                ...
                                {activeTab === 'expenses' && (
                                    expenses.length > 0 ? (
                                        expenses.map(e => (
                                            <div key={e._id} className="group p-4 bg-muted rounded-xl border border-border hover:border-primary-200 dark:hover:border-primary-500/30 transition-all">
                                                <div className="flex justify-between items-start mb-2">
                                                    <div className="flex-1">
                                                        <p className="font-bold text-card-foreground text-lg">৳{e.amount.toLocaleString()}</p>
                                                        <p className="text-sm text-muted-foreground line-clamp-2 mt-1">{e.items}</p>
                                                    </div>
                                                ...
                                                <div className="flex justify-between items-end text-sm text-muted-foreground mt-3 border-t border-border pt-3">
