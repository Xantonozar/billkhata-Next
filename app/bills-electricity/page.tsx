...
import { ElectricityIcon, ArrowLeftIcon, PlusIcon } from '@/components/Icons';
...
const ICON = <ElectricityIcon className="w-8 h-8 text-yellow-500" />;
            ...
            if (b.id === billId) {
                return {
                    ...
                    shares: b.shares.map(s =>
                        s.userId === user.id ? { ...s, status: 'Pending Approval' as const } : s
                    )
                };
            }
            return b;
            ...
            setBills(prevBills => prevBills.map(b => {
                if (b.id === billId) {
                    return {
                        ...b,
                        shares: b.shares.map(s =>
                            s.userId === user.id ? { ...s, status: 'Unpaid' as const } : s
                        )
                    };
                }
                return b;
        ...
        setBills(prev => prev.map(b => {
            if (b.id === billId) {
                return {
                    ...b,
                    shares: b.shares.map(s =>
                        s.userId === userId ? { ...s, status: 'Paid' as const } : s
                    )
                };
            }
            return b;
            ...
            setBills(prev => prev.map(b => {
                if (b.id === billId) {
                    return {
                        ...b,
                        shares: b.shares.map(s =>
                            s.userId === userId ? { ...s, status: 'Pending Approval' as const } : s
                        )
                    };
                }
                return b;
        ...
        setBills(prev => prev.map(b => {
            if (b.id === billId) {
                return {
                    ...b,
                    shares: b.shares.map(s =>
                        s.userId === userId ? { ...s, status: 'Unpaid' as const } : s
                    )
                };
            }
            return b;
    ...
    const renderManagerView = (bill: Bill) => (
        <div className="space-y-4 sm:space-y-6">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 sm:p-6 space-y-3 sm:space-y-4">
                ...
                <div>
                    <h3 className="font-bold text-xs sm:text-base font-sans text-slate-800 dark:text-white mb-2">Split Among {bill.shares.length} Members:</h3>
                    <div className="p-2 sm:p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg space-y-2">
                        {bill.shares.map(s => (
                            <div key={s.userId} className="flex justify-between items-center text-xs sm:text-sm">
                                <div>
                                    <p className="font-medium text-slate-800 dark:text-slate-200">{s.userName} - <span className="font-numeric">৳{s.amount.toFixed(0)}</span></p>
                                    <span className={`text-[10px] sm:text-xs font-semibold ${statusColors[s.status].text}`}>{s.status}</span>
                                </div>
                                <div className="flex gap-1 sm:gap-2">
                                    {s.status === 'Pending Approval' && (
                                        <>
                                            <button onClick={() => handleDenyPayment(bill.id, s.userId)} className="px-2 py-0.5 sm:py-1 text-[10px] sm:text-xs font-semibold bg-danger-500/10 text-danger-600 rounded-md hover:bg-danger-500/20">Deny</button>
                                            <button onClick={() => handleApprovePayment(bill.id, s.userId)} className="px-2 py-0.5 sm:py-1 text-[10px] sm:text-xs font-semibold bg-success-500/10 text-success-600 rounded-md hover:bg-success-500/20">ok</button>
                                        </>
                                    )}
                ...
                <div className="pt-3 sm:pt-4 border-t border-slate-200 dark:border-slate-700 flex gap-2 justify-end">
                    <button onClick={() => setEditingBill(bill)} className="px-3 py-1.5 text-xs sm:text-sm font-semibold border border-slate-300 dark:border-slate-600 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700">Edit</button>
        ...
        return (
            <div className="space-y-4 sm:space-y-6">
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 sm:p-6 space-y-3 sm:space-y-4">
                    ...
                    <div>
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Due: {bill.dueDate}</p>
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">Status: <span className={`font-semibold ${statusColors[myShare.status].text}`}>{myShare.status}</span></p>
                    </div>
                    <div className="flex gap-3">
                        {(myShare.status === 'Unpaid' || myShare.status === 'Overdue') && (
                            <button onClick={() => setConfirmingPayment(bill)} className="px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-semibold bg-gradient-success text-white rounded-md hover:shadow-lg">Pay Now</button>
                        )}
                    </div>
                </div>
            </div>
                ...
                <div className="space-y-4 sm:space-y-6">
                    <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-4">
                        <div className="flex items-center gap-2 sm:gap-4">
                            <button onClick={() => router.push('/bills/overview')} className="p-1.5 sm:p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800"><ArrowLeftIcon className="w-5 h-5 sm:w-6 sm:h-6 text-slate-600 dark:text-slate-300" /></button>
                    ...
                    <div className="flex justify-end">
                        <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-semibold bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md">
